...
清单是**查出来的，不是记下来的**——每一步都是当场问服务要的事实，所以删掉渠道之后
第一步会重新变回未完成。存一个勾选状态做不到这件事。
...
![NotifyRelay 组件分层](docs/NotifyRelay_CN_Architecture_Layers.png)

六列，依赖箭头只朝一个方向。核心域是唯一一段不知道 `dingtalk` 这种东西存在的代码，
所有通道知识都住在它右边的渠道插件层里。两条信任边界在这里看得最清楚：接入层认的是
服务手里的 Bearer Key，运维面认的是人手里的会话加 CSRF——`internal/admin` 之所以是
单独一个包，就是因为它是单独一条边界。
...
<img src="docs/NotifyRelay_CN_Architecture.png" width="1000" alt="NotifyRelay 系统架构">

一张图看完全貌，下半部分是部署形态。几处推理这套系统时容易忘掉的：后台管理界面和 API
是同一个端口，`sync: true` 的请求和默认异步走的不是队列里的同一条路径，`spool` 和
`secret` 是核心从不直接触碰的存储细节，以及 SMTP 入口不入本地队列——它校验完收件人就
交回上游，失败时用 451 让上游自己重试。
...
部署形态见「系统架构」一节那张完整图的下半部分：一个 Pod、两个端口、一个 PVC。

单实例，两个端口，一个卷。卷就是全部状态——队列、渠道配置、消息正文，**以及两个密钥**
（`keys/secret.key` 与 `keys/session.key`，首次启动生成，权限 0600）——所以备份是
"拷一个目录"，不是一张要记的清单。

> `secret.key` 丢了，库里所有渠道凭据就变成不可读的密文。密钥和数据放在一起是有意的取舍：
> 早先的版本要求它单独配置、分开存，换来的是"一份只有密文的备份和没有备份没区别"；
> 现在默认生成到数据目录里，换来的是**备份一个目录就能完整恢复**。代价写在运维手册 §2。

```bash
# Docker Compose —— 没有前置步骤。镜像自带配置，两个密钥首次启动时自己生成，
# 管理员账号在页面上创建。.env 只在你想改端口或注入渠道凭据时才需要。
docker compose up -d
...
后台**在二进制里默认关闭**（`admin.enabled: false`），**在容器镜像里默认开启**。
两个默认值不同是有意的：手工在机器上起一个服务不该顺带开出一个没人要的管理界面，
而整个镜像的意义就是"不用编辑器也能配好"，那需要界面。它能改所有通道的配置，
比通知 API 敏感得多：
...
| [`docs/08-ui-plan.md`](docs/08-ui-plan.md) | 后台界面改造计划：调查结论、五段执行顺序，以及为什么改版和翻译要合并做 |
| [`docs/i18n-inventory.md`](docs/i18n-inventory.md) | 后台文案清单：逐条数出来的 236 条界面文案、64 条渠道参数文案，以及刻意不翻译的那几类 |
