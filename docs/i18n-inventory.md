...
> **已完成**（`samples/en/` + `samples/zh/`）。落地时加了一条这里没写到的约束：
> **两种语言必须是同一个程序，只有注释不同**。重复一份文件就是两份会漂移的文件，
> 而会漂移的是代码——有人改了英文那份的头，中文那份还是旧的，拿到旧的那份的人看不出来。
> `samples_test.go` 把两份的注释剥掉后逐行比对，并有一个下限防止剥离器把整份文件吃掉。
>
> 另外两处只有实现时才会发现的：Python 的 `"""..."""` 是散文但不是注释，第一版剥离器
> 没管它，那段 TLS 警告因此成了整页唯一没翻译的段落——剥离规则补上了 `"""`。
> 三处小节分隔线在翻译时被替换串里的 `\1` 吃成了控制字符，已按英文的显示宽度补回。
