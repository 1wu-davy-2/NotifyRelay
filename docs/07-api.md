...
| `label` | `string` | ✅ | 表单标题。**永远是英文**，见下 |
| `desc` | `string` | ✅ | 表单说明。**永远是英文**，见下 |
...
> **`label` 和 `desc` 不随语言变。** 这个端点的输出是机器读的契约，不是页面文案：
> 一个按 `label` 分支的客户端不该因为调用方换了个 cookie 就拿到别的字符串。
> 后台表单显示的中文是**另加的覆盖层**——`internal/admin/i18n/paramdata.go` 里按
> `类型.参数名` 存译文，渲染时盖在 schema 的英文上，盖不到就落回英文。
> 所以加一个渠道类型**不需要动 i18n 包**（这是本项目最硬的一条边界），
> 代价是漏翻一条是测试失败而不是编译失败。
