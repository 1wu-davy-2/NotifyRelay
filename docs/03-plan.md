...
**表单由 schema 生成**（验收 #2、#11）。做法是**服务端构建全部通道类型的字段组**，
前端只负责按选中的类型显示、按 `ShowIf` 联动。

> **M5.4 修订**：字段组原先由服务端直接渲染成 HTML，前端界面迁到 React 之后改为
> `GET /api/channel-form` 返回同一份 `fieldView`。原则没变而且更强了——表单仍然是
> 服务端构建的，客户端仍然不重新推导任何规则。付出的是这条注释原本强调的那个好处：
> Go 测试不再能对真实 HTML 断言，只能对 JSON 断言；控件上的 `required`、条件字段的
> 显隐、类型选择框有没有占位项，都成了前端属性，由浏览器实测覆盖。
> 保留下来的是服务端那一半：`required` / `required_now` / `show_if_field` /
> `show_if_equals`，见 `internal/admin/ui_test.go`。
