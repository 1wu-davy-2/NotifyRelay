...
- 所有改状态的请求要求自定义头（`X-NotifyRelay-Admin`），加上 `SameSite=Lax` 的
  ...
  不是第二把凭据。
- **密码可以在后台改**（侧栏「修改密码」），改完会踢掉其它所有会话——
  改密码的常见原因是"我觉得别人知道了"，留着对方的会话等于没做这件事。
  **发起改密的那个会话保留**，否则运维分不清是改成功了还是改失败了。
  密码写在配置里（`admin.password_hash`）的部署改不了这里，页面会说清楚要改哪儿。
- **没有找回。** 忘了密码只能改数据库，或者往配置里写一个新的哈希再重启。
  找回需要第二凭据或邮件通道，两者这里都没有。
